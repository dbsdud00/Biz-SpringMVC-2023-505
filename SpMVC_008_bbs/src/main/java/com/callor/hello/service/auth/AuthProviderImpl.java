...
public class AuthProviderImpl implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		// login 할 때 입력한 username 과 password 를 추출하기
		...
		if(!username.equalsIgnoreCase("yoooy")) {
			throw new UsernameNotFoundException(username+"이 없습니다.");
		}
		...
		// 로그인 실행
		// 정상적인 사용자 이면 token 을 발행하고, session에 사용자 정보를 등록하기
		// GRANT : 권한을 부여한다.
		List<GrantedAuthority> grantList = new ArrayList<>();
		
		// USER 라는 권한을 설정하기
		/*
		 *  context.xml 에서 권한을 검사할 때
		 *  hasRole("권한") 처럼 사용한다.
...
//		grantList.add(new SimpleGrantedAuthority("ROLE_GUEST"));
		...
		// 정상적인 사용자 임을 증명하는 Token 을 발행하여 return
		// 이 Token 을 사용하여 Spring Security 는 정상적인 로그인 절차를 마무리한다.
