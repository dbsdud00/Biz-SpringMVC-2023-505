...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /*
    JS 의 false 판별
    어떤 변수의 값이 null, undefined, "", 0, NaN(Not a Number) 등의 값을 가질 때 
    변수 || 연산 을 수행하면 이 결과는 false 처럼 작동한다.
  ...
  */
  ...
  const inputButtonClickHandler = (e) => {
    ...
    // alert("추가 버튼 클릭");
    // JS code 에서 form 에 포함된 button 을 클릭했을 때
    // Server 로 데이터를 전송하라
  ...
  input_id?.addEventListener("blur", async (e) => {
    ...
    if (result === "OK") {
      ...
      message_view(INDEX.ID, "error", "* 서버 오류. 알수없는 오류입니다.!!");
      input_id.focus();
    }
  });

  // msg_box?.classList.add("ok");
  // span_box.innerHTML = value;
});
