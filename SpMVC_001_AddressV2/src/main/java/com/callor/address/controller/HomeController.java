...
//	protected AddrDao addrDao;
	 ...
	 * method 생성
	 */
	// 처음 화면에서 주소 추가 버튼을 클릭했을 때 
	// 추가 화면을 보여주는 method
	/*
	 * method 의 매개변수로 HttpSession 객체를 설정하는 순간
	 * 이 method 에서 Session 에 저장된 데이터를 참조할 수 있다. 
	 */
	...
	public String insert(Model model, HttpSession httpSession) {
		...
		// 로그인이 되어 있지 않으면 userDto 는 null 값이다.
...
//	}
	public String insert(@ModelAttribute AddrDto addrDto, HttpSession httpSession) {
		UserDto userDto = (UserDto) httpSession.getAttribute("USER");
		if(userDto == null) {
			return "redirect:/user/login?error=LOGIN";
		}
