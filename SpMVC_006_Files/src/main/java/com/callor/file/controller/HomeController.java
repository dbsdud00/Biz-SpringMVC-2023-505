...
@CrossOrigin(value= {"http://192.168.4.51:5500/","*"})
@Slf4j
@Controller
public class HomeController {
	...
	public String update(@RequestParam(value = "b_seq", required=false, defaultValue = "0")long b_seq,@ModelAttribute("BBS") BBsDto bbsDto , Model model) {
		bbsDto = bbsService.findById(b_seq);
		model.addAttribute("BBS",bbsDto);
		return "input";
	}
	@RequestMapping(value="/update", method=RequestMethod.POST)
	public String update(@ModelAttribute("BBS") BBsDto bbsDto , @RequestParam("b_file") MultipartFile b_file,
			MultipartHttpServletRequest b_files) {
	...
	public String img_delete(@RequestParam(value="b_seq", required=false, defaultValue ="0")long b_seq,
