...
public class BBsServiceImplV1 implements BBsService {
	...
	public void insert(BBsDto bbsDto, MultipartFile b_file, MultipartHttpServletRequest b_files) {
		...
		// form 에 multiple 로 설정되어 있는 input tag 의 이름(b_files)

		try {
			List<FileDto> files = fileService.filesUp(b_files);
			int result = galleryService.insert(files, bbsDto.getB_seq());
	...
	/*
	 * 파일 포함한 update 1. 새롭게 업로드된 대표파일과 원래 Table 에 저장된 파일이름 비교 2. 업로드된 파일과 Table 에
	...
	public int update(BBsDto bbsDto, MultipartFile b_file, MultipartHttpServletRequest b_files) {
		...
		if (!upload_name.isBlank() && !upload_name.equals(old_name)) {
			...
			try {
				...
				// 새로운 파일정보(파일이름, 업로드 이름) Dto 에 setting
		...
		try {
			if (files.size() > 0) {
				...
				fileList = fileService.filesUp(b_files);
				galleryService.insert(fileList, b_seq);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
