...
<body>
	...
	<form:form action="${rootPath}/rent/go/complete"
		...
		<%
		/*
		첫 화면에서 입력된 도서정보를 이 화면에서 입력하는 회원정보와 함께 묶어서 다음화면으로 전달하기 위해서는 감춰진 또는 다른 위치에 있는
		input tag 를 생성하고 그 input tag에 데이터를 담아 놓아야 한다.
			<div>
				<input hidden="hidden" name="rent_bcode" value="${RENT_WORK.rent_bcode}" />
				<input hidden="hidden" name="b_name" value="${RENT_WORK.rent_bcode}" />
				<input hidden="hidden" name="b_code" value="${RENT_WORK.rent_bcode}" />
			</div>
		하지만 Spring 에서는 SessionAttributes(), ModelAttribute() 
		그리고 String form tag 를 사용하므로써 데이터를 일일이 챙기는 코드를 생략할 수 있다.
		*/
		%>
		<div>
			<label>회원명</label>
