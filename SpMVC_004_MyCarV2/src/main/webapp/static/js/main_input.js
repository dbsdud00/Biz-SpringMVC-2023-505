...
document.addEventListener("DOMContentLoaded", () => {
  ...
  input_carnum?.addEventListener("blur", async (e) => {
    ...
    if (json.c_carnum === "NOT") {
      ...
      err_carnum.innerHTML =
        "* 차량번호가 한번도 사용하지 않는 차량번호입니다.";
      input_skm.value = "";
    } else {
  ...
  input_qty?.addEventListener("blur", (e) => {
    err_clear();
    if (!err_message(e.target, err_qty, ERROR_MESSAGE.QTY)) {
      return false;
    }
  });
  input_sdate?.addEventListener("blur", (e) => {
    err_clear();
    if (!err_message(e.target, err_sdate, ERROR_MESSAGE.SDATE)) {
      return false;
    }
  });
  input_stime?.addEventListener("blur", (e) => {
    err_clear();
    if (!err_message(e.target, err_stime, ERROR_MESSAGE.STIME)) {
      return false;
    }
  });
  input_skm?.addEventListener("blur", (e) => {
    err_clear();
    if (!err_message(e.target, err_skm, ERROR_MESSAGE.SKM)) {
      return false;
    }
  });
  input_username?.addEventListener("blur", (e) => {
    err_clear();
    if (!err_message(e.target, err_username, ERROR_MESSAGE.USERNAME)) {
      return false;
    }
  });
